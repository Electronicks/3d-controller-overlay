...
export typedef struct texture_struct
{
    GLuint id = 0;
    std::string name = "";
    std::string path;
...
export typedef struct model_struct
{
    std::string path;
...
export void loadModel(Model& m, std::string path)
{
    m.path = path;
...
export void writeInfo(Model& m, std::string path)
{
    std::string file_path = path;
...
export void deleteTexture(GLuint& id)
{
    glDeleteTextures(1, &id);
    id = 0;
}

export void loadTexture(GLuint& id, std::string path)
{
    if (id == 0)
        glGenTextures(1, &id);
...
export void drawModel(Model m, GLuint shader)
{
